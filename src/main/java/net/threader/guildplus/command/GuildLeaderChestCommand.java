...
public class GuildLeaderChestCommand implements CommandRunner {
    @Override
    public CommandResult execute(CommandContext commandContext, String[] strings) {
        ...
        GuildChestEvent event = new GuildChestEvent(Member.of(player.getUniqueId()).get(), true);
        Bukkit.getPluginManager().callEvent(event);
        if(!event.isCancelled()) {
            Inventory chest = ArmaChestPlugin.instance().getVirtualChestManager().getChest(guild.getUniqueId().toString(), 2);
            player.openInventory(chest);
            player.playSound(player.getLocation(), Sound.BLOCK_CHEST_OPEN, 1.0F, 1.0F);

            guild.broadcast(new Message.Builder().fromConfig("abriu_bau_lider", GuildPlus.instance()).addVariable("player", player.getName()).build());
        }
