...
public class GuildChangeTagCommand implements CommandRunner {
    @Override
    public CommandResult execute(CommandContext commandContext, String[] strings) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(GuildPlus.instance(), () -> {
            try(PreparedStatement st = GuildPlus.DATABASE_ACESSOR.getConnection().prepareStatement(
                    "UPDATE guilds SET tag=? WHERE unique_id=?")) {
                st.setString(1, tag);
        ...
        return CommandResult.builder().result(Result.SUCCESS).message("Tag da guilda atualizada com sucesso!").build();
    }
}
